...
function LevelCheck({ profile, onClose, onSave, onAdd, onHelp }){
  const [stage,setStage] = lvS(-1); // -1 intro · 0..sections · 99 results
  const [ans,setAns] = lvS({});
  const [skipped,setSkipped] = lvS(false);
  const savedRef = lvR(false);
  const role = (profile && profile.builder)==="spouse" ? "spouse" : "worker";
  const answered = Object.keys(ans).length;
  const res = stage===99 ? lvScore(ans, skipped) : null;
  ...
  function nextFrom(i){
    if(i+1 < LSECTIONS.length) setStage(i+1); else setStage(99);
  }
  function skipDrink(){ setSkipped(true); setStage(99); }
  ...
  function Gauge({ id, label, ok, value, okText, flagText, children }){
    return (
  ...
  if(stage===-1){
  ...
  } else if(stage>=0 && stage<LSECTIONS.length){
    const sec = LSECTIONS[stage];
    const lang = lvLang();
    ...
    body = (
      <div className="sbody lvlsec">
        ...
        <div style={{display:"flex",gap:9,marginTop:20}}>
          {sec.optional && <button className="btn line" style={{flex:1}} onClick={skipDrink}>{lt("skip")}</button>}
          <button className="btn" style={{flex:2}} disabled={!done} onClick={()=>nextFrom(stage)}>
            {stage===LSECTIONS.length-1 ? lt("finish") : lt("next")}
          </button>
        </div>
      </div>
    );
    ...
    body = (
      <div className="sbody">
        ...
        <Gauge id="well" label="gWell" ok={!f.well} value={res.who5+"/100"} okText="rWellOk" flagText="rWellFlag" />
        <Gauge id="mood" label="gMood" ok={!f.mood} value={res.phq2+"/6"} okText="rMoodOk" flagText="rMoodFlag">
        ...
        <Gauge id="worry" label="gWorry" ok={!f.worry} value={res.gad2+"/6"} okText="rWorryOk" flagText="rWorryFlag" />
        <Gauge id="conn" label="gConn" ok={!f.conn} value={res.ucla3+"/9"} okText="rConnOk" flagText="rConnFlag" />
        <Gauge id="sleep" label="gSleep" ok={!f.sleep} value={res.sleep+" "+lt("hrs")} okText="rSleepOk" flagText="rSleepFlag" />
        <Gauge id="money" label="gMoney" ok={!f.money} value={(3-res.money)+"/3"} okText="rMoneyOk" flagText="rMoneyFlag" />
        {res.audit!=null
          ? <Gauge id="drink" label="gDrink" ok={!f.drink} value={res.audit+"/12"} okText="rDrinkOk" flagText="rDrinkFlag" />
  ...
  return (
    <div className="overlay" onClick={stage===-1?onClose:undefined}>
      <div className="sheet" onClick={e=>e.stopPropagation()}>
        <div className="sh">
          <div>
            <h2>{stage===99 ? lt("resultsH") : lt("introH")}</h2>
            <div className="drv">{lt("kick")}</div>
          </div>
          <button className="closex" onClick={onClose}>×</button>
        </div>
        {stage>=0 && stage<LSECTIONS.length && (
          <div className="dockprog" style={{margin:"0 22px",border:"1px solid var(--line)"}}>
            <div className="dockbar"><i style={{width:Math.round(answered/LV_TOTAL*100)+"%"}}></i></div>
            <span>{lt("progress",{a:answered,b:LV_TOTAL})}</span>
          </div>
        )}
        {body}
      </div>
...
window.RebarLevel = { LevelCheck, LevelStrip, lvDue, lvLast };
